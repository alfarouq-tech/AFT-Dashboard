    ...
    <tr>
      ...
      <td className="pe-2 d-flex align-items-center justify-content-center gap-3 h-full mt-3">
        <BiTrash
          ...
          onKeyPress={(e) => e.key === 'Enter' && dispatch(deleteEmployee(employeeData.id))}
          tabIndex={0}
        />
        <BiEdit
          type="button"
          className={`btn btn-warning p-1 fs-3 ${editable ? 'd-none': 'd-block'}`}
        ...
        <BiSave
          type="button"
          className={`btn btn-success p-1 fs-3 ${editable ? 'd-block': 'd-none'}`}
          onClick={saveChanges}
